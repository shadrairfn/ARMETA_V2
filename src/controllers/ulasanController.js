...
const createUlasan = asyncHandler(async (req, res) => {
	...
	let { idMatkul, idDosen, idReply, idForum, judulUlasan, textUlasan, isAnonymous } = req.body;
...
const editUlasan = asyncHandler(async (req, res) => {
	...
	const [oldReview] = await db.select().from(reviews).where(eq(reviews.id_review, id_review));
...
const deleteUlasan = asyncHandler(async (req, res) => {
	...
	const [review] = await db.select().from(reviews).where(eq(reviews.id_review, id_review));
	...
	const [reviewResult, repliesResult] = await Promise.all([
		...
		db
			.select({
				...
				total_reply:
					sql`(SELECT count(*)::int FROM reviews r WHERE r.id_reply = ${reviews.id_review})`.as(
						"total_reply"
					),
				is_liked: sql`count(case when ${likeReviews.id_user} = ${userId} then 1 end) > 0`.mapWith(
					Boolean
				),
				is_bookmarked:
					sql`count(case when ${bookmarkReviews.id_user} = ${userId} then 1 end) > 0`.mapWith(
						Boolean
					),
...
const getLikeUlasan = asyncHandler(async (req, res) => {
	...
	const dataUlasan = await db
		.select({
			...
			is_liked: sql`count(case when ${allLikes.id_user} = ${currentUserId} then 1 end) > 0`.mapWith(
				Boolean
			),
			is_bookmarked:
				sql`count(case when ${allBookmarks.id_user} = ${currentUserId} then 1 end) > 0`.mapWith(
					Boolean
				),
			is_anonymous: reviews.is_anonymous,
		})
...
const getBookmarkUlasan = asyncHandler(async (req, res) => {
	...
	const dataUlasan = await db
		.select({
			...
			is_liked: sql`count(case when ${allLikes.id_user} = ${userId} then 1 end) > 0`.mapWith(
				Boolean
			),
			is_bookmarked:
				sql`count(case when ${allBookmarks.id_user} = ${userId} then 1 end) > 0`.mapWith(Boolean),
			is_anonymous: reviews.is_anonymous,
		})
