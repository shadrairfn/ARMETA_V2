...
import { reviews, reviewsForum, users } from "../db/schema/schema.js";
import { generateQueryEmbedding } from "../service/vectorizationService.js";
...
const createForum = asyncHandler(async (req, res) => {
	...
	const result = await db.execute(
		sql`INSERT INTO reviews_forum (id_user, id_subject, title, description, files, is_anonymous)
          VALUES (${userId}, ${id_subject}, ${title}, ${description}, ${filesJson}, ${!!isAnonymous})
          RETURNING *`
	);
...
const deleteForum = asyncHandler(async (req, res) => {
	...
	const [forum] = await db.select().from(reviewsForum).where(eq(reviewsForum.id_forum, id_forum));
