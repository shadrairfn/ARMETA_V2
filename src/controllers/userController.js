...
const logout = asyncHandler(async (req, res, _next) => {
	...
	await db.update(users).set({ refreshToken: null }).where(eq(users.id_user, userId));
...
const refreshAccessToken = asyncHandler(async (req, res, _next) => {
	...
	const [user] = await db.select().from(users).where(eq(users.id_user, decoded.id_user)).limit(1);
...
const updateProfile = asyncHandler(async (req, res) => {
	...
	// Cek user lama
	const [oldUser] = await db.select().from(users).where(eq(users.id_user, userId));
...
export { logout, refreshAccessToken, getCurrentUser, getUserById, updateProfile };
